...
function loadAveJsonDoc() {
    if (window.XMLHttpRequest) {
        req = new XMLHttpRequest();
    } else {
        req = new ActiveXObject("Microsoft.XMLHTTP");
    }
    req.onreadystatechange = handleResponseAve;
    req.open("GET", "jsonGetAllLettters", true);
    req.send(); 
}

// This function is called for each request readystatechange,
// and it will eventually parse the XML response for the request
function handleResponseAve() {
    if (req.readyState != 4 || req.status != 200) {
        return;
    }

    var removeavechart = document.getElementById("aveChart");
    while (removeavechart.hasChildNodes()) {
        removeavechart.removeChild(removeavechart.firstChild);
    }
    ...
    d3.json("jsonGetAllLettters", function(error, data){
